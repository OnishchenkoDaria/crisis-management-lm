...
class Scenario(Base):
    id: Mapped[int_pk]
    external_id: Mapped[str_uniq]

    # сore fields (directly from extraction JSON)
    title: Mapped[str_not_null]
    crisis_type: Mapped[str_not_null]
    severity: Mapped[str_not_null]  # low | medium | high | critical
    phase: Mapped[str_not_null]
    context: Mapped[str] = mapped_column(Text, nullable=False)
    stakeholders: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)

    # fields not yet in extraction JSON — nullable until populated
    time_pressure: Mapped[str_null_true]
    initial_statement_required: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    decision_nodes: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)
    relevant_tactics: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)
    difficulty_for_rookie: Mapped[str_null_true]  # basic | intermediate | expert
