...
@router.post("/workspaces/{workspace_id}/chats", response_model=ChatResponse,
             status_code=status.HTTP_201_CREATED, summary="Create a new chat in workspace")
...
@router.delete("/workspaces/{workspace_id}/chats/{chat_id}",
               status_code=status.HTTP_204_NO_CONTENT, summary="Delete a chat")
...
async def rename_chat(workspace_id: int, chat_id: int, body: ChatRename) -> ChatResponse:
...
@router.patch("/workspaces/{workspace_id}/chats/{chat_id}/finish", response_model=ChatResponse,
...
async def send_message(workspace_id: int, chat_id: int, body: dict) -> dict:
    chat = await _get_chat_or_404(chat_id, workspace_id)
    ...
    async with generation_lock(workspace_id, chat_id):
        # Replace with: await create_analysis(workspace_id, SituationInput(**body))
